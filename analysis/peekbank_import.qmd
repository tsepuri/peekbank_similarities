...
datasets <- get_datasets(connection = conn) |> dplyr::collect()

 # ds.get_raw_data("frank_tablet_2016", "kartushina_2021")
administrations <- get_administrations(connection = conn) |> dplyr::collect()
subjects <- get_subjects(connection = conn) |> dplyr::collect()
aoi_timepoints <- get_aoi_timepoints(rle=FALSE, connection = conn) |> dplyr::collect()
stimuli <- get_stimuli() |> dplyr::collect()
trial_types <- get_trial_types(connection = conn) |> dplyr::collect()
trials <- get_trials(connection = conn) |> dplyr::collect()
```
...
clip_data_summarized <- summarize_similarity_data(usable_trials_summarized_with_sims, extra_fields = c("dataset_name")) |> filter(dataset_name == "weaver_zettersten_2024")
#clip_data_summarized <- summarize_similarity_data(usable_trials_summarized_with_sims |> filter(14 < age & age < 24), extra_fields = c("dataset_name")) |> filter(N > 10)
